...
!  Parameters:
!
!    Input, integer ( kind = 4 ) N, the number of nodes and associated 
!    data values.  10 <= N.
!
...
!
!    Input, integer ( kind = 4 ) NQ, the number of data points to be used in
!    the least squares fit for coefficients defining the nodal functions Q(K).  
!    A recommended value is NQ = 17.  9 <= NQ <= MIN ( 40, N-1 ).
!
!    Input, integer ( kind = 4 ) NW, the number of nodes within (and defining) 
!    the radii of influence R(K) which enter into the weights W(K).  For N 
!    sufficiently large, a recommended value is NW = 32.  
!    1 <= NW <= min(40,N-1).
!
!    Input, integer ( kind = 4 ) NR, the number of rows, columns, and planes in
!    the cell grid defined in subroutine store3.  A box containing the nodes is
!    partitioned into cells in order to increase search efficiency.  
!    NR = (N/3)**(1/3) is recommended.  1 <= NR.
!
!    Output, integer ( kind = 4 ) LCELL(NR,NR,NR), nodal indices
!    associated with cells.  Refer to STORE3.
!
!    Output, integer ( kind = 4 ) LNEXT(N), next-node indices.  Refer to STORE3.
!
...
!    to be evaluated.
!
!    Input, integer ( kind = 4 ) N, the number of nodes and data values 
!    defining Q.  10 <= N.
!
...
!
!    Input, integer ( kind = 4 ) NR, the number of rows, columns and planes in
!    the cell grid.  Refer to STORE3.  1 <= NR.
!
!    Input, integer ( kind = 4 ) LCELL(NR,NR,NR), nodal indices associated 
!    with cells.  Refer to STORE3.
!
!    Input, integer ( kind = 4 ) LNEXT(N), the next-node indices.  Refer
!    to STORE3.
!
...
!    Input, real ( kind = 8 ) X(N), Y(N), Z(N), the coordinates of the nodes.
!
!    Input, integer ( kind = 4 ) NR, the number of rows, columns, and planes
!    in the cell grid.  1 <= NR.
!
!    Input, integer ( kind = 4 ) LCELL(NR,NR,NR), nodal indices associated 
!    with cells.
!
!    Input/output, integer ( kind = 4 ) LNEXT(N), next-node indices (or their
!    negatives).
!
...
!    be positive.
!
!    Output, integer ( kind = 4 ) NP, index of the nearest unmarked node to P, 
!    or 0 if all nodes are marked or NR < 1 or an element of XYZDEL is not 
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) N, the number of columns to be rotated.
!
...
!  Parameters:
!
!    Input, integer ( kind = 4 ) N, the number of nodes.  2 <= N.
!
!    Input, real ( kind = 8 ) X(N), Y(N), Z(N), the coordinates of the nodes.
!
!    Input, integer ( kind = 4 ) NR, the number of rows, columns, and planes in
!    the grid.  The cell density (average number of nodes per cell) is 
!    D = N/(NR**3).  A recommended value, based on empirical evidence, 
!    is D = 3, so NR = (N/3)**(1/3).  1 <= NR.
!
!    Output, integer ( kind = 4 ) LCELL(NR,NR,NR), a cell array such that 
!    LCELL(I,J,K) contains the index (for X, Y, and Z) of the first node (node 
!    with smallest index) in cell (I,J,K), or LCELL(I,J,K) = 0 if no nodes are
...
!
!    Output, integer ( kind = 4 ) LNEXT(N),  next-node indices such that
